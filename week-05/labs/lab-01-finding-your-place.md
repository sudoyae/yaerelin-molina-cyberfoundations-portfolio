...
```
4 packets were transmitted and received and while the latency to my gateway at first ping is 1.000 ms, the average round-trip time is 1.150 ms. I calculated the average round-trip time by adding the four ping times (1.000 + 1.200 + 1.100 + 1.300) and dividing by 4, which gives an average of 1.150 ms.
```
...
```
The ping command performed DNS resolution, translating foundry-archive.grid.local into the IP address 10.20.5.20. The host responded successfully with 0% packet loss and average latency of 2.150 ms.
```
...
```
The hostname is foundry-archive.grid.local and the IP address is 10.20.5.20
```
...
```
When you ping by hostname, DNS is needed to resolve the hostname to an IP address. When you ping by IP address, DNS isn't needed, allowing you to test network connectivity independently of DNS resolution. 
```
...
```
The Grid uses names due to DNS ability to connect a hostname to an IP address, even if the machine's IP address changes later. Tools like ping can use ICMP to test whether that machine is reachable, making it easier for cybersecurity professionals to troubleshoot those systems. For example, if IP ping works but hostname ping fails, suspect DNS because the IP address bypasses the need to resolve the hostname. 
```
...
```
The subnet mask lets your machine decide whether a destination IP address is on the same local network or a different network. If the IP address is on the same local network, the computer will send the packet directly to the device on the same local network. If the destination is outside the local network, the packet is sent to the default gateway, which forwards it toward the destination network. 
```
...
```
If you only know one, you may be limited in your ability to investigate systems and respond to incidents. Knowing both command languages allow you to perform the same tasks regardless of the environment. For example, during an incident, you might need to use PowerShell to investigate a Windows machine and bash to investigate a Linux server. 
```
...
```
https://github.com/sudoyae/yaerelin-molina-cyberfoundations-portfolio/blob/main/assets/screenshots/week-05/cli-grid-address.png?raw=true
```
