...
```
I found 11 files with 3 .log files and 8 .txt files. 5 of files were invoices. 
```
...
```
02:47 DENIED badge 4471 storeroom door. This line is suspicious to me because there was an attempted access to the storeroom during odd hours. 
```
...
```
door-access.log:08:12 
door-access.log:12:40 
door-access.log:02:47 
west-access.log:13:22
...
```
The owner, group, and others can read and write the meeting-recap.txt file.
```
...
```
1. ls -l meeting-recap.txt
2. chmod go-rw meeting-recap.txt
3. ls -l meeting-recap.txt
```
...
```
It's important to verify with ls before taking action to ensure that you don't accidentally modify or mess up the wrong file. RM is used to remove files, so we must make sure that the file we need is not accidentally deleted. 
```
...
```
Reducing six hundred lines to three is useful because it makes it easier to focus on only the important information instead of going through every single file. If needed, we will go through every single one. However, identifying the most important lines can help you find the activity more efficiently while saving hassle, time, and possibly money. 
```
...
```
Finding the file by its contents helps us identify files that contain sensitive information, even if the file name does not make it obvious. Checking the permissions lets us see who can access or modify that file. You need both skills because finding sensitive information may identify what kind of file it is, but checking permissions tells you who can access or modify it. 
```
...
```
I believe using commands such as grep with wildcards $, ?, and [] would scale the furthest into professional work because they allow you to search through many files quickly. Instead of checking every file individually, you can use patterns to narrow down the files you need and find important or suspicious information efficiently. This would save a lot of time when working with millions of files. 
```
...
![Archive investigation — find, check, lock down](https://github.com/sudoyae/yaerelin-molina-cyberfoundations-portfolio/blob/main/assets/screenshots/week-04/cli-search-investigation.png?raw=true)
