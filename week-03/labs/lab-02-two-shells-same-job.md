...
```
access-log.txt incident-42 investigation-notes-backup.txt investigation-notes.txt
```
...
| 5. Create + back up your note | touch | New-Item  |
...
```
Windows output is more detailed because it displays a structured layout with the current directory path. Bash, on the other hand, lists the contents of the current directory in plain text without any extra details.
```
...
```
The file system tree was identical because both the Linux and Powershell commands displayed the same files regardless of different commands. For example, the PowerShell command Copy-Item investigation-notes.txt invistigation-notes-backup-txt both produced the same results by creating an identical backup file in the same folder. The location of the file was the same in both outputs. 
```
...
```
The command pair that felt most different to me was Get-ChildItem and ls. I found ls more comfortable to use because it is easier to type and displays the directory contents in a simple, less cluttered format. However, I found Get-ChildItem useful because it provides additional information, such as when a file or directory was last modified and its size in bytes, without requiring another command. 
```
...
```
One difference that was visibly new to me was case sensitivity. In PowerShell, commands such as Get-Location, Get-ChildItem, and Copy-Item uses both upper and lower case letters, while Bash commands like pwd, ls, cd, and cp are short and lowercase. Bash commands were easier to remember and took less time to type. 
```
